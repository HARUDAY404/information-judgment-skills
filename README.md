...
Radar 不会拿到一个宽泛主题就立刻开始抓取。它会先用两个很轻的关卡确认：这个问题要帮助你理解或决定什么；回答它需要哪些信源、当前缺少哪些覆盖。只有某个缺失渠道确实重要时，才会请你扫码、登录或配置 API，并提前说明它能补充什么、需要什么权限以及不接入的影响。
